...
#define the function to draw the epiline in the image
def draw_epiline(img1,img2,F):
    ...
    plt.axis('off')
    #save figure
    plt.savefig('epipolar_img.png',pad_inches=0.0,bbox_inches='tight')
    plt.show()
    key = cv2.waitKey(0)
    if key == 27: #press Q 
        cv2.destroyAllWindows()
    return epiline
#define the function to get the epipole e'
def smallestsingularvector(F):#3x3
        u,s,vh = np.linalg.svd(F,full_matrices=1,compute_uv=1)
        # svd function in python is different with the svd in matlab
        #so we need to get the last row of vh
        vN = vh[vh.shape[0]-1,:]
        vN = vN.T
        return vN
def compute_epipole(F):
    ...
    return e/e[2]
#define the function to get the matches between the images.
...
#define the functions to draw the functions
def draw_matches(img1,img2):
    matches, kp1, kp2 = get_matchs(img1,img2)
    img3 = cv2.drawMatches(img1,kp1,img2,kp2,matches,None,flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
    plt.imshow(img3)
    plt.show()
#define the function :from the matchers to get the pixel points
def get_pixel_points(img1,img2):
    matches, kp1, kp2 = get_matchs(img1,img2)
    x1 = []
    x2 = []
    for i, val in enumerate(matches):
        #here 30.0 means the haming distance, we wanna get the good matches such that
        #their distance is under the max(2*min_dist, 30.0)
    ...
    x2 = np.int32(x2)
    return x1,x2
#define the function using the ransac method
...
def compute_fundamental(x1,x2):
    ...
    # because rank(F) = 2, we need to recreate F
...
def question_1():
    ...
    # F  = compute_F_8(img1,img2)
    l = draw_epiline(img1,img2,F)
    e = compute_epipole(F)
    print("F_T .  e' = {} (using groundtruth)".format(e.dot(l)))
    print("fundamental matrix in the txt is \n {}".format(F / F[2,2]))
def question_2():
    img1 = cv2.imread('chapel00.png',cv2.IMREAD_GRAYSCALE)     # queryImage
    img2 = cv2.imread('chapel01.png',cv2.IMREAD_GRAYSCALE) # trainImage
    matches, kp1, kp2 = get_matchs(img1,img2)
    F , mask = compute_F_RANSAC(img1,img2)
    l = draw_epiline(img1,img2,F)
    e = compute_epipole(F)
    print("F_T .  e' = {} (using RANSAC method)".format(e.dot(l)))
...
def question_3():
    ...
    l = draw_epiline(img1,img2,F)
    e = compute_epipole(F)
    print("F_T .  e' = {} (using 8 points method)".format(e.dot(l)))
